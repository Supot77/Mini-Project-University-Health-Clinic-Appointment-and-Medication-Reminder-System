...
const dashboardRoles: UserRole[] = ['medical', 'staff_admin'];
const audienceRoles: UserRole[] = ['patient', 'medical', 'staff_admin'];
...
function BroadcastPanel({ actorId, onSent }: { actorId: string; onSent: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    const result = await repositories.notifications.sendBroadcast({
      actorId, actorRole: 'staff_admin', notificationType, title, message,
  ...
  return (
    <Section title="ส่งประกาศ Broadcast" description="เฉพาะเจ้าหน้าที่/แอดมิน · ระบบตรึงรายชื่อผู้รับ ณ เวลาส่งและตัดผู้รับซ้ำให้อัตโนมัติ">
      <form onSubmit={submit} className="space-y-5 p-5 sm:p-6">
        ...
        {!sendAll && (
          ...
          <fieldset><legend className="text-sm font-medium text-slate-700">เลือกบทบาทผู้รับอย่างน้อย 1 กลุ่ม</legend><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">{audienceRoles.map((role) => <label key={role} className="flex min-h-11 items-center gap-2 rounded-lg bg-white px-3 py-2 text-sm text-slate-600 ring-1 ring-slate-200"><input type="checkbox" checked={selectedRoles.includes(role)} onChange={() => toggleRole(role)} className="size-4 accent-sky-600" />{roleLabels[role]}</label>)}</div></fieldset>
          </div>
        )}
...
export default function DashboardPage() {
  ...
  const [previewRole, setPreviewRole] = useState<UserRole>('staff_admin');
