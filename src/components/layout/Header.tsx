...
{ href: "/patients/search", label: "ค้นหาผู้ป่วย", icon: UserSearch, roles: ["medical"] },
{ href: "/staff/accounts", label: "จัดการข้อมูลผู้ใช้งาน", icon: UserSearch, roles: ["staff_admin"] },  
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 h-16 border-b border-white/10 bg-brand-ink text-white shadow-sm">
      <nav aria-label="เมนูหลัก" className="mx-auto flex h-full w-full max-w-[1440px] items-center gap-4 px-4 sm:px-6">
        <Link href="/" onClick={() => setMobileMenuOpen(false)} className="flex shrink-0 items-center gap-2 rounded-brand-sm font-bold tracking-tight transition-opacity hover:opacity-90 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand-accent">
        ...
        <div className="hidden min-w-0 flex-1 items-center justify-center gap-1 lg:flex xl:gap-2">
          {visibleNavigation.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={`flex min-h-10 items-center gap-2 rounded-brand-sm px-3 text-[13px] font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-accent xl:text-[15px] ${active ? "bg-white/15 text-white" : "text-brand-footer-text hover:bg-white/10 hover:text-white"}`}>
      ...
      {mobileMenuOpen && (
        <nav id="mobile-navigation" aria-label="เมนูมือถือ" className="absolute inset-x-0 top-16 border-t border-white/10 bg-brand-ink px-4 pb-5 pt-3 shadow-2xl lg:hidden">
          <div className="mx-auto grid max-w-lg gap-1">
            {visibleNavigation.map((item) => {
              const Icon = item.icon;
              const active = isActive(item.href);
              return (
                <Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)} aria-current={active ? "page" : undefined} className={`flex min-h-11 items-center gap-3 rounded-brand-button px-3 text-[15px] font-medium transition-colors ${active ? "bg-white/15 text-white" : "text-brand-footer-text hover:bg-white/10 hover:text-white"}`}>
            ...
            <div className="mt-2 border-t border-white/10 pt-3">
              {isAuthenticated ? (
                <div className="flex items-center justify-between gap-3">
                  <Link href="/profile" onClick={() => setMobileMenuOpen(false)} className="flex min-h-11 min-w-0 items-center gap-3 rounded-brand-button px-3 text-[15px] text-brand-footer-text hover:bg-white/10">
                ...
                <Link href="/login" onClick={() => setMobileMenuOpen(false)} className="flex min-h-11 items-center justify-center gap-2 rounded-brand-button bg-brand-accent px-4 text-[15px] font-bold text-brand-ink">
                  <LogIn className="h-4 w-4" aria-hidden="true" />เข้าสู่ระบบ
                </Link>
              )}
            </div>
