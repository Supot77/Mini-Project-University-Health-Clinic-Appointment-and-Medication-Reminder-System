...
export default function AppointmentWorkspace({ role: initialRole = 'patient', allowRolePreview = true }: { role?: PreviewRole; allowRolePreview?: boolean }) {
  ...
  const [service, setService] = useState('ตรวจโรคทั่วไป');
  ...
  const todayQueue = snapshot.appointments.filter((item) => slotFor(item).date === DEMO_TODAY && (role !== 'medical' || slotFor(item).doctorId === DEMO_DOCTOR_ID));
  const baseList = role === 'patient' ? myAppointments.filter((item) => tab === 'history' ? ['completed', 'cancelled', 'rejected', 'no_show'].includes(item.status) : ['pending', 'confirmed', 'in_progress'].includes(item.status)) : tab === 'queue' || role === 'medical' ? todayQueue : snapshot.appointments.filter((item) => ['pending', 'confirmed'].includes(item.status));
  const visible = baseList.filter((item) => (statusFilter === 'all' || item.status === statusFilter) && `${item.patient} ${item.id} ${item.queue} ${slotFor(item).doctor} ${slotFor(item).service}`.toLowerCase().includes(search.trim().toLowerCase()));
  const bookingSlots = snapshot.slots.filter((slot) => slot.date === date && slot.service === service);
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        ...
        {allowRolePreview && <label className="grid gap-1.5 text-xs font-medium text-slate-500">มุมมองตัวอย่าง<PaiSelect value={role} onChange={(value) => changeView(value as PreviewRole)} placeholder="เลือกมุมมอง" ariaLabel="มุมมองตัวอย่าง" options={[{ value: 'patient', label: 'ผู้ป่วย' }, { value: 'medical', label: 'แพทย์/เภสัชกร' }, { value: 'staff_admin', label: 'เจ้าหน้าที่/แอดมิน' }]} /></label>}
      </div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
        {[
          ...
          { label: 'รออนุมัติ', value: (role === 'patient' ? myAppointments : role === 'medical' ? todayQueue : snapshot.appointments).filter((item) => item.status === 'pending').length, icon: Clock3, color: 'bg-amber-50 text-amber-600' },
      ...
      {bookingOpen && <section id="booking-form" aria-labelledby="booking-heading" className={`${panelClass} mb-6 border-sky-200`}>
        ...
        <form onSubmit={book} className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
          <div className="space-y-5">
            <div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm font-medium">บริการ<PaiSelect value={service} onChange={(value) => { setService(value); setSlotId(''); }} placeholder="เลือกบริการ" ariaLabel="บริการ" className="scroll-mt-24" options={Array.from(new Set(snapshot.slots.map((slot) => slot.service))).map((name) => ({ value: name, label: name }))} /></label><label className="grid gap-2 text-sm font-medium">วันที่ต้องการนัด<input required type="date" min="2026-09-08" max="2026-09-21" className={inputClass} value={date} onChange={(event) => { setDate(event.target.value); setSlotId(''); }} /></label></div>
      ...
      {rescheduling && <section className={`${panelClass} mb-6 border-amber-200`} aria-labelledby="reschedule-heading">
        ...
        <form className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); if (apply(role === 'patient' ? repository.acceptProposal(rescheduling.id, proposedSlotId) : repository.propose(rescheduling.id, proposedSlotId, proposalReason))) setRescheduleId(null); }}>
          <label className="grid min-w-0 gap-2 text-sm font-medium">รอบใหม่ของแพทย์เดิม<PaiSelect value={proposedSlotId} onChange={setProposedSlotId} placeholder="เลือกรอบที่ว่าง" ariaLabel="รอบใหม่ของแพทย์เดิม" className="min-w-0 scroll-mt-24" options={alternativeSlots.map((slot) => ({ value: slot.id, label: slotDescription(slot) }))} /></label>
          {role === 'staff_admin' && <label className="grid gap-2 text-sm font-medium">เหตุผลในการเลื่อน<input required maxLength={300} className={inputClass} value={proposalReason} onChange={(event) => setProposalReason(event.target.value)} placeholder="เช่น แพทย์งดตรวจในวันเดิม" /></label>}
      ...
      <section className={panelClass} aria-labelledby="appointment-list-heading">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><h2 id="appointment-list-heading" className="text-lg font-semibold text-slate-900">{role === 'patient' ? 'รายการนัดหมายของฉัน' : role === 'medical' ? 'ผู้ป่วยในความรับผิดชอบวันนี้' : 'รายการนัดหมายและคิว'}</h2><p className="mt-1 text-xs text-slate-500">{role === 'patient' ? 'ติดตามสถานะและรายละเอียดการนัดหมาย' : '7 กันยายน 2569 · รายการคิวตัวอย่าง'}</p></div><span className="text-sm text-slate-500">{visible.length} รายการ</span></div>
        {role !== 'medical' && <div className="my-5 flex flex-wrap gap-2" role="group" aria-label="ประเภทนัดหมาย">{(role === 'patient' ? [{ id: 'upcoming', label: 'นัดที่กำลังจะมาถึง' }, { id: 'history', label: 'ประวัตินัดหมาย' }] : [{ id: 'queue', label: 'คิวตรวจวันนี้' }, { id: 'requests', label: 'คำขอและนัดล่วงหน้า' }]).map(({ id, label }) => <button key={id} className={`min-h-11 rounded-xl px-4 text-sm font-medium focus-visible:outline-2 focus-visible:outline-sky-600 ${tab === id ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`} aria-pressed={tab === id} onClick={() => { setTab(id); setStatusFilter('all'); }}>{label}</button>)}</div>}
        <div className="mb-5 mt-5 grid gap-3 sm:grid-cols-[1fr_180px]"><label className="relative"><span className="sr-only">ค้นหานัดหมาย</span><Search className="absolute left-3 top-3 h-5 w-5 text-slate-400" aria-hidden="true" /><input className={`${inputClass} pl-10`} placeholder={role === 'patient' ? 'ค้นหาเลขนัด แพทย์ หรือบริการ' : 'ค้นหาชื่อผู้ป่วย เลขนัด หรือเลขคิว'} value={search} onChange={(event) => setSearch(event.target.value)} /></label><label><span className="sr-only">กรองสถานะนัด</span><PaiSelect value={statusFilter} onChange={setStatusFilter} placeholder="ทุกสถานะ" ariaLabel="กรองสถานะนัด" options={[{ value: 'all', label: 'ทุกสถานะ' }, ...Object.entries(statusLabels).map(([value, label]) => ({ value, label }))]} /></label></div>

        <div className="space-y-3">{visible.map((item) => {
          const slot = slotFor(item);
          return <article key={item.id} aria-label={`นัดหมาย ${item.id}`} className="rounded-xl border border-slate-200 p-4 transition hover:border-slate-300"><div className="flex flex-col gap-4 lg:flex-row lg:items-center"><div className="flex min-w-0 flex-1 items-start gap-4"><div className="flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-xl bg-slate-50 text-slate-700"><span className="text-xl font-bold">{slot.date.slice(-2)}</span><span className="text-[10px]">ก.ย. 69</span></div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold text-slate-900">{role === 'patient' ? slot.service : item.patient}</h3><Status value={item.status} /></div><p className="mt-1 text-sm text-slate-600">{slot.doctor}{role !== 'patient' && ` · ${slot.service}`}</p><p className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500"><span className="flex items-center gap-1"><Clock3 className="h-3.5 w-3.5" aria-hidden="true" />{slot.start}–{slot.end} น.</span><span>{item.id}</span><span>คิว {item.queue}</span></p>{item.proposal && <p className="mt-2 text-xs text-amber-700">มีข้อเสนอเลื่อนนัด · รอผู้ป่วยตอบรับ</p>}</div></div>
            <div className="flex flex-wrap gap-2 lg:justify-end">
              ...
              {role === 'staff_admin' && item.status === 'pending' && <><button className={secondaryButtonClass} onClick={() => apply(repository.changeStatus(item.id, 'rejected', role))}>ไม่อนุมัติ</button><button className={primaryButtonClass} onClick={() => apply(repository.changeStatus(item.id, 'confirmed', role))}>อนุมัตินัด</button></>}
              {role === 'staff_admin' && ['pending', 'confirmed'].includes(item.status) && !item.proposal && <button className={secondaryButtonClass} onClick={() => openReschedule(item)}>เสนอเลื่อนนัด</button>}
              ...
              {role === 'medical' && item.status === 'in_progress' && <Link href="/records" className={primaryButtonClass}>ไปหน้าบันทึกผลตรวจ<ArrowRight className="h-4 w-4" aria-hidden="true" /></Link>}
            </div></div>
