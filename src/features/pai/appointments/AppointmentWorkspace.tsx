      ...
      {bookingOpen && <section id="booking-form" aria-labelledby="booking-heading" className={`${panelClass} mb-6 border-sky-200`}>
        ...
        <form onSubmit={book} className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
          <div className="space-y-5">
            <div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm font-medium">บริการ<select ref={bookingField} className={`${inputClass} scroll-mt-24`} value={service} onChange={(event) => { setService(event.target.value); setSlotId(''); }}>{Array.from(new Set(snapshot.slots.map((slot) => slot.service))).map((name) => <option key={name}>{name}</option>)}</select></label><label className="grid gap-2 text-sm font-medium">วันที่ต้องการนัด<input required type="date" min="2026-09-08" max="2026-09-21" className={inputClass} value={date} onChange={(event) => { setDate(event.target.value); setSlotId(''); }} /></label></div>
