...
function RecordEditor({ data, busy, selectedId, save }: {
  ...
  return <form className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" onSubmit={async (e) => {
    ...
    <fieldset disabled={busy} className="space-y-4 p-5 sm:p-6">
      <label className="block text-sm font-medium text-slate-700">คิวที่กำลังตรวจ<ClinicSelect value={chosen?.id ?? ''} onChange={(value) => { setAppointmentId(value); setDiagnosis(''); setAdvice(''); setItems([]); }} placeholder="เลือกคิว" ariaLabel="คิวที่กำลังตรวจ" options={pending.map((a) => ({ value: a.id, label: `คิว ${a.queue_number ?? '—'} · ${a.patient}` }))} /></label>
      ...
      {items.map((item, index) => <fieldset key={index} className="grid gap-3 rounded-xl border border-slate-200 p-4 sm:grid-cols-2">
        ...
        <label className="text-sm">การใช้ยากับอาหาร<ClinicSelect value={item.meal} onChange={(value) => update(index, { meal: value })} placeholder="เลือกวิธีใช้" ariaLabel="การใช้ยากับอาหาร" options={['ก่อนอาหาร', 'หลังอาหาร', 'พร้อมอาหาร', 'ไม่ขึ้นกับมื้ออาหาร'].map((meal) => ({ value: meal, label: meal }))} /></label>
