...
describe('Pai records local preview', () => {
  it('shows only the patient’s completed records and only assigned patients in medical view', async () => {
    ...
    expect((await repository.list('medical')).map(item => item.id)).toEqual(['REC-001', 'REC-002', 'REC-003', 'REC-004']);
  });

  it('requires a saved diagnosis for completion and reveals the completed draft to its patient', async () => {
    const repository = createRecordsDemoRepository();
    const before = await repository.list('medical');
    expect(repository.complete('REC-001', 1).ok).toBe(false);
    expect(await repository.list('medical')).toEqual(before);
  ...
  it('preserves dispensed quantity and instructions while recording a reason and new version', async () => {
    const repository = createRecordsDemoRepository();
    const before = (await repository.list('medical')).find(item => item.id === 'REC-002')!;
    ...
    const after = (await repository.list('medical')).find(item => item.id === 'REC-002')!;
  ...
  it('keeps preview instances and returned snapshots isolated', async () => {
    ...
    const snapshot = await first.list('medical');
    snapshot[1].prescriptions[0].dispensed = 999;
    expect((await first.list('medical'))[1].prescriptions[0].dispensed).toBe(4);
    first.saveDraft('REC-001', { diagnosis: 'ผลตรวจ', advice: '', prescriptions: [] }, 1);
    expect((await second.list('medical'))[0].diagnosis).toBe('');
  });
});
